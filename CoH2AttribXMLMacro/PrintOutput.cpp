...
PrintOutput::PrintOutput(HWND hWnd, HWND hEditor)
{
...
LRESULT PrintOutput::SendEditor(UINT Msg, WPARAM wParam /* = 0 */, LPARAM lParam /* = 0 */)
{
	return directFn(directPtr, Msg, wParam, lParam);
}

void PrintOutput::SetAStyle(int style, COLORREF fore, COLORREF back /* = PRINTOUTPUT_COLOR_WHITE */, bool bold /* = false */, bool italic /* = false */, int size /* = -1 */, const char *face /* = 0 */) {
...
void PrintOutput::Notify(SCNotification* notification)
{
	int tensOfLines;
...
void PrintOutput::SetWidth(int newWidth)
{
...
void PrintOutput::InitializeEditor(void)
{
	...
	SetAStyle(STYLE_DEFAULT, PRINTOUTPUT_COLOR_FG_DEFAULT, PRINTOUTPUT_COLOR_BG_DEFAULT, false, false, 10, "Courier New");
	SendEditor(SCI_STYLECLEARALL);	// Copies global style to all others

	SetAStyle(PRINTOUTPUT_STYLE_DEFAULT, PRINTOUTPUT_COLOR_FG_DEFAULT, PRINTOUTPUT_COLOR_BG_DEFAULT, false, false, 10, "Courier New");
}
